...
ask_player(Game):-
    nl,write('Color for Player 1? (o or x)'),nl,
    read(Player_1),nl,
...
move_player1(Board,S,S1):-
      ...
      Opponent=o,
      Counter=0,
      check_winpose(Player,Opponent,NextBoard,Counter,S,S1).


move_player2(NextBoard,S,S1):-
      write('Player 2 Move? (o)'),nl,
      read(Pos),nl,
      Player = o,
      ...
      Opponent=x,
      Counter=0,
      check_winpose(Player,Opponent,Board,Counter,S,S1).
...
check_winpose(Player,Opponent,Board,Counter,S,S1):-
     ...
     Opp_list=[],
     Neighbour=[],
     winpose(Y,Player,Opponent,Board,Counter,Opp_list,Neighbour,S,S1),
    Counter1 is Counter+1,
    check_winpose(Player,Opponent,Board,Counter1,S,S1);
    Opponent = o, move_player2(Board,S,S1);
    Opponent = x,   move_player1(Board,S,S1).


winpose(Y,Player, Opponent,Board,Pos,Opp_list,Neighbour,S,S1):-
    Y= Opponent,
     collect_Neighbour(Opponent,Board,Pos,Opp_list,Neighbour,S,S1),
    write('End neighbour list');
    Counter1 is Pos+1,
    check_winpose(Player,Opponent,Board,Counter1,S,S1),write('miss'),nl.
...
collect_Neighbour(Opponent,Board,Pos,Opp_list,Neighbour,S,S1):-
    ...
    Counter=0,!,
    check_opp(Pos,Opponent,Board,New_Opp_list,Neb_List,Counter,S,S1);
    check_neighbour_list(Opponent,Board,Opp_list,Neighbour,S,S1).

check_opp(Pos,Opponent,Board,Opp_list,Neighbour,Counter,S,S1):-
    Counter<25,

    member_chk(Counter,Neighbour,X),
    X = true,
    nth0(Counter,Board,Y),
    is_mem(Y,Opponent,Board,Counter,Opp_list,Neighbour,S,S1),
    Counter1 is Counter+1,
    check_opp(Pos,Opponent,Board,Opp_list,Neighbour,Counter1,S,S1);
    Counter<25,
   Counter1 is Counter+1,
    check_opp(Pos,Opponent,Board,Opp_list,Neighbour,Counter1,S,S1);
    Counter=25,
...
check_neighbour_list(Opponent,Board,Opp_list,Neighbour,S,S1):-
   ...
   write('END NEB LIST :'),
    ...
    Counter=0,
    len(Neb_List,Len1),
    is_surounded_by_player(Counter,Len1,Opponent,Board,Opp_list,Neb_List,S,S1).

is_surounded_by_player(Counter,Len ,Opponent,Board,Opp_list,Neb_List,S,S1):-
    Counter<Len,
    ...
    is_surounded_by_player(Counter_1,Len ,Opponent,Board,Opp_list,Neb_List,S,S1);
    Counter=Len,
    Counter2=0,
    len(Opp_list,Len1),
  remove_opponent(Counter2,Len1 ,Opponent,Board,Opp_list,Neb_List,S,S1).


remove_opponent(Counter,Len1 ,Opponent,Board,Opp_list,Neb_List,S,S1):-
     Counter<Len1,
    nth0(Counter,Opp_list,M),
    replace(Board,M,-, NextBoard),
    Counter1 is Counter+1,
    remove_opponent(Counter1,Len1 ,Opponent,NextBoard,Opp_list,Neb_List,S,S1);
     Counter=Len1,
     update_board(Opponent,Board,S,S1).

update_board(Opponent,Board,X,X2):-
    sleep(3),
     nl,show_board(Board),

     Opponent = o,incr(X, X1),write('Player X :'),write(X1),nl,write('Player O :'),write(X2),nl, move_player2(Board,X1,X2);
    Opponent = x,incr(X2, X3),write('Player X :'),write(X),nl,write('Player O :'),write(X3),nl, move_player1(Board,X,X3).
...
is_mem(Y,Opponent,Board,Mem,Opp_list,Neighbour,S,S1):-


    Y=Opponent,
    delete(Neighbour,Mem,Neb_List),
    collect_Neighbour(Opponent,Board,Mem,Opp_list,Neb_List,S,S1).
